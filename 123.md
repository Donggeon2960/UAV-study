...
classDiagram
    ...
    namespace Assistance_Core_System {
        ...
        class HelpContentManager {
            +getAdaptiveHelp(context)
            +getManualHint(context)
    ...
    namespace Feature_Services {
        ...
        class FraudDetector {
            ...
            +updateRiskList()
            +checkAppSafety(pkgName)
            +showOverlayWarning()
        }
        class TrafficService {
            +getBusArrivalInfo(stationId)
            +parseToEasyFormat(data)
        }
        class WelfareService {
            +getCustomizedWelfare(profile)
            +filterByLocationAndAge()
        }
    }
    ...
    %% Relationships (간소화하여 레이아웃 최적화)
    User "1" *-- "1" UserProfile
    User --> ActionLog
    
    BehaviorMonitor --> User
    BehaviorMonitor --> HelpContentManager
    HelpContentManager --> HelpContent
    
    User --> ChatbotService
    User --> TutorialSystem
    
    FraudDetector ..> ExternalFraudAPI
    FraudDetector --> User
    
    TrafficService ..> ExternalTrafficAPI
    WelfareService ..> ExternalWelfareAPI
    WelfareService ..> UserProfile
    
    Administrator --> HelpContentManager
```




## 2. 클래스 설명 (Class Specifications)

본 시스템의 주요 클래스 명세는 Java API Documentation 표준 형식을 따릅니다.

### 2.1 Package: User Management
...
### 2.2 Package: Assistance Core System
